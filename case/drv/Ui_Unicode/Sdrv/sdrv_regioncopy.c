...
void Sdrv_RegionCopy(region_t *sregion, region_t *tregion)
{
	...
	if ((sregionram.x) >= (tregionram.x))
	{
		...
		for(k=j;k<pagenum;k++)
		{
			...
			{
				DisplayBuffer[tregionram.x+i+k*Display_Length]=DisplayBuffer[sregionram.x+i+k*Display_Length];
			}
		}
		//把暂时显存区的数据送到LCM
	...
	else
	{
		...
		for(k=j;k<pagenum;k++)
		{
			...
			{
				DisplayBuffer[tregionram.x+i-1+k*Display_Length]=DisplayBuffer[sregionram.x+i-1+k*Display_Length];
			}
		}
		//把暂时显存区的数据送到LCM
		//Hdrv_SetWindow(&tregionram);
