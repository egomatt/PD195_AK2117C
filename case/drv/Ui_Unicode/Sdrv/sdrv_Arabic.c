...
#pragma userclass(hconst=BBD_ARABIC_CONST)
const KEY_INFO far  _aKeyInfo[] =
    {
...
#pragma renamecode(?PR?BBC_ARABIC)
/*********************************************************************/
...
{
	uint16 LenOut = 0, IgnoreNextCharacter = 0;
	...
	while (i<total_len)
	{
		...
		NextChar = *(int *)(str+i+2);
		NNextChar = *(int *)(str+i+4);
