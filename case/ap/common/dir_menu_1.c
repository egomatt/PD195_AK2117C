...
uint16 dir_ui_directory(file_location_t *location, uint8 type)
{
 ...
 //   uint16 ScrollCounter2=0;
    ...
    {
       init_dir_buf1(location->fselTotal, 1); 
    }
    ...
    dir_list_active = 0;
    //dir_list_active=dir_list_active+dir_total_num+1;//longjin
    cur_group = 0;
    selmode = 0;
...
//			updownflag = 1;
            ...
            case AP_KEY_MODE | AP_KEY_UP://返回操作
            #if(MACHINE_MODEL == 1)
            ...
            {   
				...
				{*/
	                ...
	                if (result != NULL)
	                {
                    	...
                    	//exFat 添加，先恢复direntry以更新全路径，再在外面获取全路径，之后再setlocation
                    	FS_GetOrSetDir_Exfat(location->filename, 0, EXFAT_SET_CURDIR_FILENAME);
                    	FS_SetCurDirEntry(location->DirEntry);
	                    return result;
	                }
	                else
	                {
